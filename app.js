const yesBtn = document.querySelector('#yesBtn');
    ...
    yesBtn.addEventListener('click', function() {
      initialContent.style.display = 'none'; // Ocultar el contenido inicial
      victoryAlert.style.display = 'block'; // Mostrar la div de victoria
      startFireworks();
    });
    ...
    function loadJson() {
      ...
      fetch('json-dance.json')  // Asegúrate de que el archivo esté en la misma carpeta
          .then(response => {
              if (!response.ok) {
          ...
          .then(data => {
              ...
              const danceMoves = data.dances.map(dance => `<li>${dance.name}: ${dance.description}</li>`).join('');
              resultContent.innerHTML = `<ul>${danceMoves}</ul>`;
          })
          .catch(error => {
              console.error('Error en el fetch:', error);
              resultContent.innerHTML = '<p>Error cargando el archivo JSON.</p>';
          });
  }
  

  function startFireworks() {
    window.oncontextmenu = function () { return false; } // Bloquear menú

    var canvas = document.getElementById('canvas'); // Obtener el elemento por ID
    var ctx = canvas.getContext('2d'); // Obtener el contexto de 2d para gráficos
    var frameRate = 60.0; // Tasa de fotogramas por segundo
